...
async function runAllTests() {
  ...
  // ----------------------------------------------------------
  // 3. 清理
  //    CONNECT 隧道会在 mock 服务器上残留 socket，
  //    直接 close() 会永远等待 —— 先强杀所有连接再关闭
  // ----------------------------------------------------------
  await relay.stop();
  console.log('  [cleanup] relay 已停止');
  if (typeof mockUpstream.closeAllConnections === 'function') {
    mockUpstream.closeAllConnections();
  }
  if (typeof mockTarget.closeAllConnections === 'function') {
    mockTarget.closeAllConnections();
  }
  console.log('  [cleanup] mock 连接已强杀');
  await new Promise((r) => { mockUpstream.close(r); mockTarget.close(r); });
  console.log('  [cleanup] mock 服务器已关闭');
