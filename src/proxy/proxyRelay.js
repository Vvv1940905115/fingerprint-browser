...
class ProxyRelay {
  ...
  /**
   * 跟踪隧道相关 socket（client 与上游两侧），在 close/error 时自动移除
   */
  _trackSocket(socket) {
    if (!this.server) {
      // relay 已停止，直接拒绝新连接
    ...
    const untrack = () => this.tunnelSockets.delete(socket);
  ...
  stop() {
    return new Promise((resolve) => {
      if (this.server) {
        // 先销毁自行跟踪的隧道 socket（CONNECT 升级 socket 不受
        // closeAllConnections 管理，不销毁它们 close() 回调永不触发）
        for (const socket of this.tunnelSockets) {
          socket.destroy();
        }
        this.tunnelSockets.clear();
        // 再断开普通 HTTP keep-alive 连接
  ...
  socks5Connect(targetHost, targetPort, clientSocket, head) {
    ...
    // 跟踪上游侧 socket
    this._trackSocket(upstreamSocket);
