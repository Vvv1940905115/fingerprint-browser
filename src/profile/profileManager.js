...
class ProfileManager {
  ...
  // ---------- 分组管理（独立持久化到 groups.json） ----------

  _loadGroups() {
    try {
      const arr = JSON.parse(fs.readFileSync(this.groupsFile, 'utf-8'));
      return Array.isArray(arr) ? arr.map(g => String(g).trim()).filter(Boolean) : [];
    } catch {
      return [];
    }
  }
  ...
  _saveGroups() {
    fs.writeFileSync(this.groupsFile, JSON.stringify(this.groups, null, 2), 'utf-8');
  }

  // 分组名登记（已存在则忽略）；环境保存时自动调用，保证下拉列表完整
  _ensureGroup(name) {
    const n = String(name || '').trim();
    if (n && !this.groups.includes(n)) {
      this.groups.push(n);
      this._saveGroups();
    }
  }

  // 合并 groups.json 与环境中实际使用的分组（兼容历史数据）
  listGroups() {
    const set = new Set(this.groups);
  ...
  // 删除分组：同时把该分组下的环境改为"未分组"
  deleteGroup(name) {
    const n = String(name || '').trim();
    if (!n) throw new Error('分组名不能为空');
    this.groups = this.groups.filter(g => g !== n);
    this._saveGroups();
    this.profiles.forEach(p => {
      if ((p.group || '').trim() === n) {
        p.group = '';
  ...
  create(config) {
    ...
    if (profile.group) this._ensureGroup(profile.group);
  ...
  update(id, updates) {
    ...
    if (updates.browser !== undefined) profile.browser = updates.browser;
    if (updates.kernelVersion !== undefined) profile.kernelVersion = updates.kernelVersion;
    if (updates.os !== undefined) profile.os = updates.os;
    if (updates.group !== undefined) {
      profile.group = updates.group;
      if (profile.group) this._ensureGroup(profile.group);
    }
    if (updates.labels !== undefined) profile.labels = updates.labels;
    if (updates.tags !== undefined) profile.tags = updates.tags;
    if (updates.fingerprint !== undefined) profile.fingerprint = updates.fingerprint;  // 整体替换，避免旧字段残留
